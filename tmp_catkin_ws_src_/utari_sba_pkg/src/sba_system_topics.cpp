...
public: // public methods
	SbaTopics(ros::NodeHandle nh) // constructor
	{
		...
		VbmeInput = nh.subscribe("/vbme/QuestData", 100, &SbaTopics::SBA_callback, this);  // input directly from QuEst
...
private: // private methods
	...
	void SBA_callback(const vbme_msgs::VbmeData_msg::ConstPtr& msg)
	{
		...
		std::vector<Eigen::Vector4d> Points3d;    				// vector of points, for logging only
		...
		Vector2d proj;
		std::vector<Vector2d> projs0; 							// vector of projections, cam0, for logging only
		std::vector<Vector2d> projs1; 							// vector of projections, cam1, for logging only
		...
		for (int i = 0; i < (int)msg->X1.size(); i++) 			// number of feature points
		{
			...
			//~ proj(1) = msg->X1[i].y;
			proj(0) = msg->MatchList.Points[i].FlowArray.front().x; 	// raw feature points
			proj(1) = msg->MatchList.Points[i].FlowArray.front().y;
			projs0.push_back(proj);
			//~ std::cout << "Cam0 Proj: " << std::endl << proj << std::endl;
			...
			sys.addMonoProj( 0 ,  ((CallbackN*5) + i ) , proj); // for chaining removed  
		}
		// camera 1
		for (int i = 0; i < (int)msg->X2.size(); i++) 			// number of feature points
		{
			...
			//~ proj(0) = msg->X2[i].x;
			//~ proj(1) = msg->X2[i].y;
			proj(0) = msg->MatchList.Points[i].FlowArray.back().x;
			proj(1) = msg->MatchList.Points[i].FlowArray.back().y;
			projs1.push_back(proj);
			//~ std::cout << "Cam1 Proj: " << std::endl << proj << std::endl;
			...
			sys.addMonoProj( ( (CallbackN) + 1 ) , ((CallbackN*5) + i ) , proj); // for chaining removed
		}
		// /////////////////////////////////////////////////////////////
		...
		//~ if (myfileNodes.good()) 								
		//~ {
			//~ // printf("Num of nodes %zd \n", sys.nodes.size());
			//~ // for (int i = 0; i < 2; i++)
			//~ for (int i = 0; i < int(sys.nodes.size()); i++)
			//~ {
		...
		//~ {
			...
			//~ {
				//~ // for (int j = 0; j < 2; j++)
				//~ for (int j = 0; j < int(sys.tracks[i].projections.size()); j++)
				//~ {
					...
					//~ std::cout << "2d pt: " << std:: endl << sys.tracks[i].projections[j].kp << std::endl;
		...
		// /////////////////////////////////////////////////////////////
		// create raw single input logfiles
		ofstream myfileNodesRaw; 									
		ofstream myfile3dPtsRaw;
		ofstream myfile2dPtsRaw;
		myfileNodesRaw.open ("/tmp/sba_topics_in_nodes_raw.csv", std::fstream::app);
		myfile3dPtsRaw.open ("/tmp/sba_topics_in_3dpts_raw.csv", std::fstream::app);
		myfile2dPtsRaw.open ("/tmp/sba_topics_in_2dpts_raw.csv", std::fstream::app);
		ofstream myfileNodesRawLinked; 									
		ofstream myfile3dPtsRawLinked;
		ofstream myfile2dPtsRawLinked;
		myfileNodesRawLinked.open ("/tmp/sba_topics_in_nodes_rawLinked.csv", std::fstream::app);
		myfile3dPtsRawLinked.open ("/tmp/sba_topics_in_3dpts_rawLinked.csv", std::fstream::app);
		myfile2dPtsRawLinked.open ("/tmp/sba_topics_in_2dpts_rawLinked.csv", std::fstream::app);
		ClearBuff(BuffData, Size);
		// code will crash (later in code?) if you write to file without calling good before....
		if (myfileNodesRaw.good()) 								
		{
			if (FirstLoop)
			{
				sprintf(BuffData, "node, %d, %f, %f, %f, %f, %f, %f, %f, \n", 0, trans0(0), trans0(1), trans0(2), rot0.w(), rot0.x(), rot0.y(), rot0.z());
				myfileNodesRaw << BuffData;
				
				myfileNodesRawLinked << BuffData;
				ClearBuff(BuffData, Size);
			}
			CurIndexCam++;
			sprintf(BuffData, "node, %d, %f, %f, %f, %f, %f, %f, %f, \n", CurIndexCam, trans1(0), trans1(1), trans1(2), rot1.w(), rot1.x(), rot1.y(), rot1.z());
			myfileNodesRaw << BuffData;
		...
		{
			ROS_WARN("Could not open myfileNodesRaw");
		}
		
		if (myfile2dPtsRaw.good())
		{
			for (int i = 0; i < 5; i++)
			{
				sprintf(BuffData, "2d pts, %d, from node, %d, is valid, %d,  %f, %f, %f, \n", CurIndexPt3d+i, 0, 1, 
				projs1[0](0), projs1[0](1), 0.0);
				myfile2dPtsRaw << BuffData;
				ClearBuff(BuffData, Size);
				sprintf(BuffData, "2d pts, %d, from node, %d, is valid, %d,  %f, %f, %f, \n", CurIndexPt3d+i, CurIndexCam, 1, 
				projs1[1](0), projs1[1](1), 0.0);
				myfile2dPtsRaw << BuffData;
				ClearBuff(BuffData, Size);
				
				// linked points assume there is only ever 5 3d pts, they will link back to these
				sprintf(BuffData, "2d pts, %d, from node, %d, is valid, %d,  %f, %f, %f, \n", i, 0, 1, 
				projs1[0](0), projs1[0](1), 0.0);
				myfile2dPtsRawLinked << BuffData;
				ClearBuff(BuffData, Size);
				sprintf(BuffData, "2d pts, %d, from node, %d, is valid, %d,  %f, %f, %f, \n", i, CurIndexCam, 1, 
				projs1[1](0), projs1[1](1), 0.0);
				myfile2dPtsRawLinked << BuffData;
		...
		if (myfile3dPtsRaw.good())
		{
			for (int i = 0; i < 5; i++)
			{
				sprintf(BuffData, "3d pts, %d, %f, %f, %f, %f, \n", CurIndexPt3d, Points3d[i](0), Points3d[i](1), Points3d[i](2), Points3d[i](3));
				CurIndexPt3d++;
				myfile3dPtsRaw << BuffData;
				if (CurIndexCam < 3)
				{
					myfile3dPtsRawLinked << BuffData;
				}
				ClearBuff(BuffData, Size);
			}
		}
		...
		{
			ROS_WARN("Could not open myfile3dPtsRaw");
		}	
		
		FirstLoop = false;
		
		myfileNodesRaw.close();
		myfile3dPtsRaw.close();
		myfile2dPtsRaw.close();
		myfileNodesRawLinked.close();
		myfile3dPtsRawLinked.close();
		myfile2dPtsRawLinked.close();
		
		ROS_WARN("RETURN FROM CALLBACK EARLY!!!");
		return;
