...
In this paper, scalar values are represented by \underline{underlined} variables,
i.e. $\underline{a},\underline{x},\underline{y},\underline{z}$.
...
Approximated vectors are represented as normal vectors with a \textit{tilde} on top
to signify its \textit{approximation} designation, i.e.  $\mathbf{\widetilde{a},\widetilde{x},\widetilde{y},\widetilde{z}}$.
The same notation is applied to matrices and scalars.
Estimated vectors are represented as normal vectors with a \textit{hat} on top
to signify its \textit{estimation} designation, i.e.  $\mathbf{\widehat{a},\widehat{x},\widehat{y},\widehat{z}}$.
The same notation is applied to matrices and scalars.
...
acting onto robot's rigid body.


First, we calculate the \textit{proper acceleration}, $a_{proper}$, which presents
forces acting onto robot's rigid body. We subtract gravity from the absolute acceleration
or \textit{coordinate acceleration}, $a_{abs}$

and its \textit{Body Frame} acceleration, $a$.
We remove Earth's gravity from system dynamics.

\begin{equation}
\label{eq:1}
a_{proper} = {C}^{I}_{B}(a_{abs} - g),
\end{equation}


Where matrix ${C}^{I}_{B}$ represents the orthonormal rotation from \textit{inertial frame},
...
\begin{equation}
\label{eq:2}
\widetilde{a} ~or~ f = a + \omega_{a}
\end{equation}


\begin{equation}
\label{eq:3}
\widetilde{\omega} = \omega + \omega_{\omega}
\end{equation}
...
\begin{equation}
\label{eq:4}
\transpose{z} :=  \left[r~v~\omega~q \right],
\end{equation}


Where the observed quaternion state is used to compute the corresponding state
rotation matrix, ${C}^{I}_{B}$.
It is important to note that the observation data is treated as the groundtruth.
...
\textit{observation} model. Variables $\omega_{f}$ and $\omega_{h}$ represent the
\textit{process} and \textit{observation} noise, respectively. Vector $u$
represent input to the system.

\subsection{Estimation State Definition}

The estimation state is defined by the robot's linear position, $r$, and velocity,
$v$, and the body frame orientation in quaternion space.

\begin{equation}
\label{eq:5}
\transpose{x} :=  \left[r~v~q \right]
\end{equation}
