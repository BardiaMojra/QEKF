...
class dmm:
  ...
  def get_images(self, imgpath, benchtype):
  ...
  def get_calib_matrix(self, benchtype, dataDir, benchnum ):
    ...
    if benchtype == 'KITTI':
      ...
      K = np.array([884.9574219,    0.0,        634.0410934,
                      ...
                      0.0,          0.0,          1.0       ], dtype=np.float64)
      K = K.reshape((3,3))
      dp = np.array([-0.2297238599,0.1703723682,-0.09885776185])
      tp = np.array([-0.0002223016917,-0.0003623410498])
      ...
      if benchnum <= 5:
        K = np.array([517.3,    0.0,   318.6,
                        ...
                        0.0,    0.0,     1.0], dtype=np.float64)
        K = K.reshape((3,3))
        dp = np.array([0.2624,  -0.9531,  1.1633], dtype=np.float64)
        tp = np.array([-0.0054,  0.0026], dtype=np.float64)
      elif benchnum == 11:
        K = np.array([535.4,      0.0,     320.1,
                        0.0,    539.2,     247.6,
                        0.0,      0.0,       1.0,], dtype=np.float64)
        K = K.reshape((3,3))
        dp = np.array([0.0,   0.0,   0.0], dtype=np.float64)
        tp = np.array([0.0,   0.0], dtype=np.float64)
      else:
        K = np.array([520.9,     0.0,    325.1,
                        0.0,   521.0,    249.7,
                        0.0,     0.0,      1.0], dtype=np.float64)
        dp = np.array([0.2312,   -0.7849,  0.9172], dtype=np.float64)
        tp = np.array([-0.0033,   0.0001], dtype=np.float64)
    else:
      assert False, 'unknown benchtype '+benchtype
...
dataset.tTru = tTru;
  ...
  def format_data(self):
    ...
    if self.name=="dataset-iphone1_clean" and self.ext=='csv':
      ...
      eprint(lhead+'Err--->> invalid name and/or ext!/n/n', file=sys.stderr)
      exit()
    ''' common df section '''
...
def plot_quat_vs_quat(quat_A_df,
  ...
  if output_dir is None:
    eprint(lhead+'Err--->> no output directory assigned for figure: '+figname+' ...../n/n')
...
def plot_df(df:pd.DataFrame,
  ...
  for n, ax in enumerate(axs.flatten()):
    ...
    if col in t_cols:
      ...
      eprint(lhead+'Err--->> column label missing for figure: '+figname+' ...../n/n')
...
def plot_df_grp(df:pd.DataFrame,
  ...
  for n, ax in enumerate(axs.flatten()):
    ...
    if col in t_cols:
      ...
      eprint(longhead+'Err--->> column label missing for figure: '+figname+' ...../n/n')
  fig.subplots_adjust(wspace=0.05)
  ax.set_xlabel('time')
  fig.legend()
  # set title
