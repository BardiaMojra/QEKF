...
def main():
  if sys.argv[1] is not None and isinstance(int(sys.argv[1]), int) is True:
    ...
    print(shead+'user input (int): '+str(_TEST_ID)+' ---> '+datasets[_TEST_ID])
...
def run(data:str):
  ...
  residual_df = get_losses(residual_df, dset.outDir)
  fignum+=1;
