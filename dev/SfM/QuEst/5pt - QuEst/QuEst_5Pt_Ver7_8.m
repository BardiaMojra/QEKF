...
[V3, ~] = eig(B3);
Ve = [V1, V2, V3];
% for now remove all the imaginary solutions
% Remove duplicate complex eigenvectors
Vy = imag(Ve);
imagIdx = sum(abs(Vy),1) > 10*eps; % add isfinite(z(Vy)) % check finite
Viall = Ve(:,imagIdx);
...
z = X5(4,:) ./ w4;
Q = [w; x; y; z];
QNrm = sqrt(sum(Q.^2,1)); % Normalize s.t. each column of Q has norm 1
