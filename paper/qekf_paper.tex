...
where \(\tilde{q}_{k+1}\) represent the current orientation estimation based on
measured angular velocity \(\tilde{\omega}_{k+1}\) and \(\hat{\transpose{C}}_{k+1} \).


\begin{equation}
        \tilde{q}_{k+1} = \text{exp}\left(\Delta t  \tilde{\transpose{C}}_{k+1} \cdot \tilde{\omega}_{k+1} \right)
\end{equation}


It is important to note that in each iteration \(\hat{\transpose{C}}_{k+1} \) is
read from the \(z\) matrix or the observation
% \
